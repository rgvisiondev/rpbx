...
export function NewsletterPageForm({ className, ...props }: React.ComponentProps<"div">) {
  ...
  // Guards to prevent Turnstile callback loops / duplicate submits
  ...
  async function submitWithToken(token: string) {
    ...
    try {
      ...
      // Optional: reset widget so user can submit again later
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email) return setErrorMsg("Please fill out all fields");
    ...
    // Trigger Turnstile; token arrives through onVerify -> submitWithToken
          ...
          <form onSubmit={handleSubmit} className="p-6 md:p-8">
            <FieldGroup>
              <div className="flex flex-col gap-2 text-center">
                <h1 className="text-2xl font-bold">Business Marketplace Expert Insights</h1>
                <p className="text-muted-foreground text-balance">
                  Join our monthly newsletter for expert insights on selling your business,
                  business valuations, seller financing strategies, and the latest businesses for sale.
                </p>
              </div>
              ...
              <Field>
                <TurnstileWidget
                  ref={turnstileRef}
                  action="newsletter"
                  onVerify={(token: string) => submitWithToken(token)}
                  onError={() => {
                    // Important: don't unlock here; just show message

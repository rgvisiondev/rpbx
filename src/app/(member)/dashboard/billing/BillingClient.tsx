...
export default function BillingClient() {
  ...
  useEffect(() => {
    (async () => {
      const res = await fetch("/api/billing/rows");
  ...
  return (
    <div className="w-full lg:w-[1140px] mx-auto py-10 px-5">
      ...
      <div className="bg-white border rounded-xl p-4">
        ...
        <button onClick={openPortal} className="mt-4 px-4 py-2 rounded-full bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] hover:cursor-pointer text-white transition">
          Open Billing Portal
        </button>
      </div>
    </div>

...
function PricingTab(props: PricingTabProps) {
  ...
  const recurringCents = props.yearly
    ? (props.price.yearly ?? props.price.monthly)
    : (props.price.monthly ?? props.price.yearly);

  const recurringPeriod = props.yearly
    ? props.price.yearly != null
      ? "/yr"
      : props.price.monthly != null
        ? "/mo"
        : ""
    : props.price.monthly != null
      ? "/mo"
  ...
  const href = props.checkoutLookup
    ? `/subscribe/${encodeURIComponent(props.checkoutLookup)}${isTrial ? `?trial=${trialDays}` : ""}`
    : "#";

  const canCheckout = props.loggedIn ? !!chosenPriceId : !!props.checkoutLookup;

  const recurringPriceLabel =
    recurringCents != null
      ? `$${(recurringCents / 100).toFixed(0)}${recurringPeriod}`
      : "";
      ...
      <div className="relative flex flex-col h-full p-6 rounded-2xl bg-white border border-slate-200 shadow shadow-slate-950/5 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl">
        {(isTrial || props.popular) && (
          <div className="absolute top-0 right-0 mr-6 -mt-4">
            <div className="inline-flex items-center text-xs font-semibold py-1.5 px-3 bg-[#60BC9B] text-white rounded-full shadow-sm shadow-slate-950/5">
              {isTrial ? `${trialDays} Days Free` : "Most Popular"}
            </div>
          </div>
        )}
        ...
        <div className="mb-5">
          ...
          {isTrial ? (
            <div className="mb-4">
              <div className="flex items-end gap-2">
                ...
                <span className="text-slate-900 font-bold text-5xl leading-none">
                  0
                </span>
              ...
              <div className="mt-1 flex items-center gap-1 text-sm text-slate-500">
                <span>Then {recurringPriceLabel}</span>

                <Tooltip>
                  <TooltipTrigger className="cursor-pointer">ⓘ</TooltipTrigger>
                  <TooltipContent>
                    Your subscription starts free. After {trialDays} days, it
                    renews at {recurringPriceLabel}. You can cancel anytime
                    before renewal.
                  </TooltipContent>
                </Tooltip>
              </div>
          ...
          <Link
            ...
            className={`w-full inline-flex justify-center whitespace-nowrap rounded-full px-4 py-3 text-sm font-medium text-white transition-colors duration-150 ${
              canCheckout
                ? "bg-[#60BC9B] hover:bg-[var(--color-primary-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#60BC9B] cursor-pointer"
        ...
        <ul className="text-slate-600 text-sm space-y-3 grow">
          {(props.features.length
            ...
            <li key={i} className="flex items-center">
              <svg
                className="w-3 h-3 fill-[#60BC9B] mr-3 shrink-0"
                viewBox="0 0 12 12"
              >
...
export default function PricingTable({ dark, loggedIn }: PricingTableProps) {
  ...
  const plans = useMemo(() => {
    ...
    for (const p of prices) {
      const stem =
        (p.lookup_key?.replace(/_(monthly|yearly)$/i, "") ||
          p.productName.toLowerCase().replace(/\s+/g, "_")) ??
        "plan";
      ...
      const order = Number.isFinite(rawOrder as number)
        ? (rawOrder as number)
        : 999;
      ...
      if (!g.features.length && p.metadata?.features) {
        g.features = p.metadata.features
          .split("|")
          .map((s) => s.trim())
          .filter(Boolean);
      }
    ...
    return arr.sort(
      (a, b) =>
        a.sortOrder - b.sortOrder || a.planName.localeCompare(b.planName),
    );
  }, [prices]);
  ...
  const visiblePlans = useMemo(() => {
    return plans
      .filter(
        (p) =>
          p.planName !== "Boosted Listing" &&
          p.planName !== "Business Owner Lite (old)",
      )
      .filter((p) => {
        ...
        <div className="relative flex w-full p-1 bg-white rounded-full">
          ...
          <button
            className={`relative flex-1 text-sm font-medium h-8 rounded-full  ${
              !isAnnual
                ? "text-white"
                : "text-slate-500 hover:text-[#60BC9B] cursor-pointer"
            }`}
          ...
          >
            Annual{" "}
            <span className={isAnnual ? "text-[#E5FFF8]" : "text-[#60BC9B]"}>
              Save 20%
            </span>
          </button>
        </div>
